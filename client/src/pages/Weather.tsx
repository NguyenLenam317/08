...
// Declare global JSX namespace to fix intrinsic element errors
declare global {
  namespace JSX {
                    ...
                    <CardContent className="p-6">
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        {(forecastData.daily?.time || []).map((date: string, i: number) => {
                          ...
                          return (
                            <div key={date} className="flex flex-col items-center p-3 bg-gray-50 rounded-lg overflow-hidden w-full max-w-[150px]">
                              <p className="text-sm font-medium overflow-hidden text-ellipsis whitespace-nowrap w-full text-center max-w-full truncate">{day}</p>
                              <span className="material-icons text-2xl my-2">{weatherIcon}</span>
                              <div className="flex items-center">
                                <span className="material-icons text-sm mr-1">air</span>
                                <span className="text-xs overflow-hidden text-ellipsis whitespace-nowrap max-w-[60px] truncate">{windSpeed} km/h</span>
                              </div>
                            </div>
                          );
                        })}
